...
class MonitorImpl(monitor: JMonitor, naming: Naming) extends Monitor {
  ...
  override def meter(name: String): Meter =
    new MeterImpl(monitor.newMeter(name))

  override def counter(name: String): Counter =
    new CounterImpl(monitor.newCounter(name))

  override def timer(name: String): Timer =
    new TimerImpl(monitor.newTimer(name))
  ...
  override def gauge[A](name: String)(value: () => A): Gauge[A] =
    new GaugeImpl[A](monitor.newGauge(name, new Supplier[A] {
      override def get(): A = value()
    }))

  override def histogram(name: String): Histogram =
    new HistogramImpl(monitor.newHistogram(name))
}
